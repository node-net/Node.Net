    ...
    public class Traverser
    {
        ...
        private void Traverse(object value)
        {
            if(!object.ReferenceEquals(null,value))
            {
                ...
                {
                    System.Collections.IEnumerable ienumerable = KeyValuePair.GetValue(value) as System.Collections.IEnumerable;
                    if (!object.ReferenceEquals(null, ienumerable))
                    {
                        foreach (object item in ienumerable)
                        {
                            object itemValue = KeyValuePair.GetValue(item);
                            if (!metaData.ContainsKey(item)) metaData[itemValue] = new Node.Net.Json.Hash();
                            metaData[itemValue]["Parent"] = KeyValuePair.GetValue(value);
                            Traverse(item);
                        }
                    }
                }
