...
namespace Node.Net.Factory.Internal.TypeFactories
{
    ...
    class IAngleFactory : Factories.Generic.TypeFactory<IAngle>, ITypeFactory<IAngle>, IFactory
    {
        ...
        public IAngle Create(object source) { return new ConcreteAngle { Angle = GetAngle(source) }; }

        private double GetAngle(object value)
        {
            if (value == null) return 0.0;
            if (value.GetType() == typeof(string)) return GetAngle(value.ToString());
            return Convert.ToDouble(value);
        }

        private double GetAngle(string value)
        {
