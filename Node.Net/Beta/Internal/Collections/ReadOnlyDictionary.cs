...
namespace Node.Net.Beta.Internal.Collections
{
	...
	{
		public ReadOnlyDictionary(IDictionary value)
		{
			data = value;
		}
		...
		public IDictionary Load(Stream stream)
		{
			...
			if (data == null) throw new InvalidOperationException("Unable to load");
		...
		{
			foreach (var key in data.Keys) { array.SetValue(data[key], index); ++index; }
		}
		...
		{
			return new ReadOnlyDictionaryEnumerator(this);
		}

		private class ReadOnlyDictionaryEnumerator : IDictionaryEnumerator
		{
			private readonly ReadOnlyDictionary readOnlyDictionary;
			private readonly Dictionary<int, string> indexedKeys;
